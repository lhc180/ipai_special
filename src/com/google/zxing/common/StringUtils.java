 ...
 * @return Boolean
 */
//    System.out.println("判断是不是GBK");
    private static Boolean isGBK(byte[] bytes) {
        Boolean isGb2312 = false;
        CharsetDetector charDect = new CharsetDetector();


        String[] probableSet = new String[0];
        try {
            probableSet = charDect.detectChineseCharset(new ByteArrayInputStream(bytes));
        } catch (IOException e) {
            e.printStackTrace();
        }
